...
if (isset($_SESSION['ctfcookies'])) {
    ...
    $requeteverif = $connexion->prepare("SELECT key3 FROM timepython WHERE cookie = ?");
    ...
    if ($valeur1 != NULL) {
        ...
        $requeteverif->close();
        $requete = $connexion->prepare("SELECT valeur_1, result_1, valeur_2,result_2, valeur_3, result_3 FROM `valeur python` WHERE nom = 'partie3'");
        $requete->execute();
