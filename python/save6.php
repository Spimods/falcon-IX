...
if (isset($_SESSION['ctfcookies'])) {
    ...
    $requete = $connexion->prepare("SELECT time6 FROM timepython WHERE cookie = ?");
    ...
    $requete2->execute();
    $requete3 = $connexion->prepare("SELECT time4, time5 FROM timepython WHERE cookie = ?");
    ...
    $requete3->close();
    $requete4 = $connexion->prepare("UPDATE python SET time_flag_2 = ?, flag2 = 1 WHERE cookie = ?");
    $requete4->bind_param("ss",$time, $idcookie);
    $requete4->execute();    
    $requete5 = $connexion->prepare("UPDATE timepython SET time6 = ?, key6 = 1  WHERE cookie = ?");
