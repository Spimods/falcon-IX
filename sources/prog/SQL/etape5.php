...
if (isset($_SESSION['ctfcookies'])) {
    ...
    $requeteverif = $connexion->prepare("SELECT key5 FROM timeprogsql WHERE cookie = ?");
...
<body>
    <div class="editor">
            <div id="editor" style="left: -10%; height: 210px; width: 30%; margin-bottom: 1em;">UPDATE users SET name = 'Jack Son' WHERE id = 2;</div>
<div id="tooltip">
            <p>Informations :</p>
            <ul>
                <li>Cette instruction SQL met à jour la valeur de la colonne "name" dans la table "users" pour l'enregistrement ayant un identifiant (id) égal à 2, en remplaçant l'ancienne valeur par 'Jack Son'.</li>
            </ul>
        </div>
    ...
    function simulateQueryExecution(query) {
        ...
        try {
            if (query.toLowerCase().includes('update') && query.toLowerCase().includes('set') && query.toLowerCase().includes('where')) {
                const setValues = query.match(/set\s+(.*)\s+where/i)[1];
                const whereClause = query.match(/where\s+(.*)/i)[1];

                const setPairs = setValues.split(',');
                const updateData = {};
                setPairs.forEach(pair => {
                    const [column, value] = pair.split('=').map(item => item.trim());
                    updateData[column.trim()] = value.trim().replace(/'/g, '');
                });

                const whereCondition = whereClause.trim();
                let rowUpdated = false;

                database.users.forEach(row => {
                    if (eval(whereCondition)) {
                        Object.keys(updateData).forEach(column => {
                            row[column] = updateData[column];
                        });
                        rowUpdated = true;
                    }
                });
