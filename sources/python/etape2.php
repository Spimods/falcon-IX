...
require '../connect.php';
date_default_timezone_set('Europe/Paris');if (isset($_SESSION['ctfcookies'])) {
    $valeurCookie = $_SESSION['ctfcookies'];
    $valeurCookieNOM = $_SESSION['ctfNOM'];
    $requeteverif = $connexion->prepare("SELECT key2 FROM timepython WHERE cookie = ?");
    ...
    if ($valeur1 != NULL) {
        ...
        $requeteverif->close();
        $requete = $connexion->prepare("SELECT valeur_1, result_1, valeur_2,result_2, valeur_3, result_3 FROM `valeur python` WHERE nom = 'partie2'");
        $requete->execute();
