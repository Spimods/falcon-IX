...
if (isset($_SESSION['ctfcookies'])) {
    ...
    $requeteverif = $connexion->prepare("SELECT key6 FROM timepython WHERE cookie = ?");
    ...
    if ($valeur1 != NULL) {
        ...
        $requeteverif->close();
        $requete = $connexion->prepare("SELECT valeur_1, result_1, valeur_2,result_2, valeur_3, result_3 FROM `valeur python` WHERE nom = 'partie6'");
        $requete->execute();
...
window.onload = function() {
    ...
    function redirect(){
        window.location.href = "save6.php?code=" + encodeURIComponent(ace.edit("editor").getValue());
    }
    function restart(){
...
`), 1);
    ...
    function saveEditor() {
        ...
        document.cookie = "editor6=" + encodeURIComponent(editor);
    }
