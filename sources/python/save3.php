...
if (isset($_SESSION['ctfcookies'])) {
    ...
    $sql = "SELECT key1, key2 FROM timepython WHERE cookie = ?";
    $resultat = $connexion->prepare($sql);
    $resultat->bind_param("s", $idcookie);
    $resultat->execute();
    $resultat->bind_result($key1, $key2);
    $resultat->fetch();
    if ($key1 == NULL or $key1 == NULL) {
        if ($key1 == NULL) {
            header('Location: timestartetape1.php'); 
        } elseif ($key2 == NULL) {
        ...
        $resultat->close();
        $requete = $connexion->prepare("SELECT time3 FROM timepython WHERE cookie = ?");
        ...
        $code = $_GET['code'];
        $lignes = explode("\n", $code);
        $lignes_de_code = array_filter($lignes, function ($ligne) { return !trim($ligne) || strpos(trim($ligne), '#') !== 0;});
        ...
        $requete2->execute();
        $requete3 = $connexion->prepare("SELECT time1, time2 FROM timepython WHERE cookie = ?");
        ...
        $requete3->close();
        $requete4 = $connexion->prepare("UPDATE python SET time_flag_1 = ?, flag1 = 1 WHERE cookie = ?");
        $requete4->bind_param("ss",$time, $idcookie);
        $requete4->execute();
        $requete5 = $connexion->prepare("UPDATE timepython SET time3 = ?, key3 = 1  WHERE cookie = ?");
...
<body>
    <div class="container">
        ...
        <button class="button" onClick='location.href="../python.php"'>Niveau Suivant <svg xmlns="http://www.w3.org/2000/svg" class="svgfleche" class="bi bi-chevron-double-right" viewBox="0 0 16 16"> <path fill-rule="evenodd" d="M3.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L9.293 8 3.646 2.354a.5.5 0 0 1 0-.708z"/> <path fill-rule="evenodd" d="M7.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L13.293 8 7.646 2.354a.5.5 0 0 1 0-.708z"/> </svg></button>
    </div>
