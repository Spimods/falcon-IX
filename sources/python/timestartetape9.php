...
if (isset($_SESSION['ctfcookies'])) {
    ...
    $requete = $connexion->prepare("SELECT `key9` FROM `timepython` WHERE `cookie`= ?");
    ...
    if ($valeur1 != NULL) {
        ...
        $requete->close();
        $requeteUpdate = $connexion->prepare("UPDATE timepython SET time9 = ? WHERE cookie = ?");
        $requeteUpdate->bind_param("ss", $tempsDebut, $valeurCookie);
        $requeteUpdate->execute();
        header('Location: etape9.php');
    }
}
exit();
?>