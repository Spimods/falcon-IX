...
if (isset($_SESSION['ctfcookies']) ) {
    ...
    $requete->execute();
    $cle = $liste["$nom"];
    $requeteuptade = $connexion->prepare("UPDATE timepython SET $cle = 1 WHERE cookie = ?");
    $requeteuptade->bind_param("s", $idcookie);
    $requeteuptade->execute();
    if ($nom == "etape1" or $nom == "etape2" or $nom == "etape3") {
        $requete2 = $connexion->prepare("UPDATE python SET flag1 = 'triche' WHERE cookie = ?");
    } elseif ($nom == "etape4" or $nom == "etape5" or $nom == "etape6") {
        $requete2 = $connexion->prepare("UPDATE python SET flag2 = 'triche' WHERE cookie = ?");
    } elseif ($nom == "etape7" or $nom == "etape8" or $nom == "etape9") {
        $requete2 = $connexion->prepare("UPDATE python SET flag3 = 'triche' WHERE cookie = ?");
    }
...
<body>
    <div class="container">
        ...
        <p style="display: inline-flex;">
            ...
            <span class="quit"><button id="btn" onclick="window.location.href = '../python.php';">Quitter</button></span>
        </p>
    </div>
</body>
</html>
