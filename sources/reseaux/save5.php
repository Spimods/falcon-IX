...
if (isset($_SESSION['ctfcookies'])) {
    ...
    if ($valeur1 == 1) {
        ...
        $requeteverif->close();
        $requete = $connexion->prepare("SELECT time5 FROM timersociaux WHERE cookie = ?");
        ...
        $requete->close();
        $requete2 = $connexion->prepare("INSERT INTO score (nom, note,  timetotal, cookie, etape) VALUES (?, ?, ?, ?, 'rsociaux')");
        $requete2->bind_param("ssss",$name,$notetime,  $time, $idcookie);
        $requete2->execute();
        $requete3 = $connexion->prepare("SELECT time1, time2, time3, time4 FROM timersociaux WHERE cookie = ?");
        $requete3->bind_param("s",$idcookie);
        $requete3->execute();
        $requete3->bind_result($time1, $time2, $time3, $time4,);
        $requete3->fetch();
        ...
        if (preg_match('/(\d+)h(\d+)min(\d+)sec/', $time1, $matches)) {
            ...
            $heure = $val1 + $val12 + $val13 + $val14 + $heure;
            $min = $val2 + $val22 + $val23 + $val24  + $minute;
            $sec = $val3 + $val32 + $val33 + $val34 + $seconde;
            if ($sec > 60){
                $sec = $sec - 60;
                $min = $min + 1;
            }
        ...
        $requete3->close();
        $requete4 = $connexion->prepare("UPDATE rsociaux SET time_flag_1 = ?, flag1 = 1 WHERE cookie = ?");
        $requete4->bind_param("ss",$time, $idcookie);
        $requete4->execute();    
        $requete5 = $connexion->prepare("UPDATE timersociaux SET time5 = ?, key5 = 1  WHERE cookie = ?");
...
<head>
    ...
    <style>
        ...
        .currency-soft-3d__front {
            ...
            background-image: url('../images/pieceresseaux1.png');
...
<body>
    ...
    Bien joué pour avoir brillamment terminé la tâche sur les réseaux sociaux de l'Ozanam CyberQuest !<br>
    Votre rapidité et précision dans la gestion des réseaux sociaux démontrent un talent exceptionnel. Continuez ainsi, votre succès est assuré.<br>
    <a class='fin' href="../reseaux.php">Terminer</a>
    </div>
    ...
    nombre = 0
        ...
        function casse() {
            ...
            if (nombre == 0) {
                ...
                back.style.backgroundImage = "url(../images/pieceresseauxcasse1.png)";
                back2.style.backgroundImage = "url(../images/pieceresseauxcasse1.png)";
            } else if (nombre == 1) {
                back.style.backgroundImage = "url(../images/pieceresseauxcasse2.png)";
                back2.style.backgroundImage = "url(../images/pieceresseauxcasse2.png)";
            } else if (nombre == 2) {
                back.style.backgroundImage = "url(../images/pieceresseauxcasse3.png)";
                back2.style.backgroundImage = "url(../images/pieceresseauxcasse3.png)";
            } else if (nombre == 3) {
                back.style.backgroundImage = "url(../images/pieceresseauxcasse4.png)";
                back2.style.backgroundImage = "url(../images/pieceresseauxcasse4.png)";
            } else if (nombre == 4) {
