...
class CommandPrompt extends Component{
  ...
  async evaluate(cmd){
    if (cmd === '') return
    let args = cmd.split(' ')
    if(this[args[0]]) await this[args[0]](args.slice(1));
  ...
  render(){
    return(
      <textarea className="command" contenteditable="true" onKeyDown={this.input} value={this.state.text} />
    )}
}
...
function Menubar(){
  return (
    <div className='window__menubar'>
      <div className="window__program">
        <img className="window__logo" src="https://devblogs.microsoft.com/commandline/wp-content/uploads/sites/33/2018/09/msdos-logo.png" />
...
ReactDOM.render(<TerminalWindow />,
